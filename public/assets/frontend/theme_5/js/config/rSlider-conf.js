...
let service_swiper = new Swiper(".js-swipe-service", {
    ...
    spaceBetween: 16,
    slidesPerView: 3.75,
    breakpoints: {1190: {slidesPerView: 1.45,}},
    speed: 500,
    observer: true,
    observeParents: true,
});
let card_other_swiper = new Swiper(".card--other__swipe", {
    autoplay: {delay: 3000},
    navigation: {nextEl: ".card-other-next", prevEl: ".card-other-prev",},
    breakpoints: {1190: {slidesPerView: 1.5,}},
    spaceBetween: 16,
    freeMode: true,
    slidesPerView: 4,
    speed: 500,
...
let swipe_card = new Swiper(".js--card__swipe", {
    autoplay: {delay: 3000},
    breakpoints: {1190: {freeMode: true,}},
    freeMode: true,
    spaceBetween: 16,
    slidesPerView: 1.5,
    speed: 500,
...
$(document).ready(function () {
    ...
    if (route_is === 'getStoreCard' || route_is === 'showDetailCard' || route_is === 'showListCard') {
        $.ajax({
            url: '/store-card/get-telecom', type: 'GET', success: function (res) {
                if (res.status) {
    ...
    if (route_is === 'showListCard' || route_is === 'showDetailCard') {
        $.ajax({
            url: '/store-card/get-amount', type: 'GET', data: {telecom: card_is}, success: function (res) {
                if (res.status) {
                    let data = res.data;
                    if (route_is === 'showListCard') {
                        let new_bc = '';
                    ...
                    if (route_is === 'showDetailCard') {
                        ...
                        data.forEach(function (card) {
                            if (kFormatter(card.amount) === value_is) {
                                ...
                                html_current += `    <div class="scratch-card card--single col-8 col-lg-12">`;
    ...
    let data_send = {amount: 0, telecom: '', quantity: 0, _token: $('meta[name="csrf-token"]').attr('content'),};
    if (width > 1199) {
        ...
        $('.js-send-data').on('click', function () {
            $.ajax({
                url: '/mua-the', type: 'POST', data: data_send, success: function (res) {
